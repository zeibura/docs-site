...
<!--
The end result of both of these two options will be the same, which is that a `ZoweKeyring` will have been created, a Zowe `localca` certificate authority will have been created and added to the keyring, and this `localca` will have been used to self sign a Zowe certificate for SSL encryption as well as a jwtsecret certificate used for encrypting the SSO JSON Web Token (JWT).
-->

### ZWEKRING JCL and zowe-setup-certificate.sh

When Zowe is installed a PDS Sample Library `SZWESAMPLIB` is created 
...
If you submit the `ZWEKRING` JCL as is it will default to option 3, namely that it will create a localca and use this to self sign a newly created certificate, both of which it will put into a `ZoweKeyring` keyring.  
