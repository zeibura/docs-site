...
**Make everything below just be about ZWEKRING** 
...
If you are using RACF and submit the JCL member `ZWEKRING` as-is it is configured for scenario 3. above, namely that it will create a keyring called `ZoweKeyring` with a self signed certificate labelled `ZoweCert`.  

If you with to perform scenario 1. or 2. above, or your security manager is ACF/2 or TopSecret you should edit the JCL variables at the beginning of `ZWEKRING` and review and edit all the commands that are valid for your security manager.  
...
If you are using RACF to check that the keyring has been successfully created run the command `RACDCERT LISTRING(ZoweKeyring) ID(ZWESVUSR)`.  The result shown shows the ZoweKeyring containing a certificate authority `localca` that has been used to self sign the Zowe certificate `ZoweCert`, as well as a certificate used to encrypt Zowe's single sign-on JSON Web Token (jwt) named `jwtsecret`.  
...
--------------------------------   ------------   --------   -------
localca                            CERTAUTH       CERTAUTH     NO   
ZoweCert                           ID(ZWESVUSR)   PERSONAL     YES  
jwtsecret                          ID(ZWESVUSR)   PERSONAL     NO   
```
If you with to use different names to the default ones depending on how you have created the keyring and certificates

- `ZWEKRING`: The keyring name is controlled with the variable `SET ZOWERING='ZoweKeyring`, the certificate name `ZoweCert` name is specified with the `SET LABEL='ZoweCert'` statement, and the certificate authority name `localca` with the line `SET LOCALCA='localca'`
- `zowe-create-certificates.sh`: 


**Make everything below just be about zowe-create-certificates.env** 
...
3. Truststore and self signed certificates

<!-- TODO JRW -->

```
apiml_cm.sh --action trust-zosmf has failed. See /u/tstradm/zowe/logs/zowe-setup-certificates-2020-07-30-04-59-13.log for more details
ERROR: z/OSMF is not trusted by the API Mediation Layer. Make sure ZOWE_ZOSMF_HOST and ZOWE_ZOSMF_PORT variables define the desired z/OSMF instance.
ZOWE_ZOSMF_HOST=S0w1.DAL-EBIS.IHOST.COM   ZOWE_ZOSMF_PORT=10443
You can also specify z/OSMF certificate explicitly in the ZOSMF_CERTIFICATE environmental variable in the zowe-setup-certificates.env file.
```
